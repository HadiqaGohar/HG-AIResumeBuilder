...
//   );
// }
// src/app/upload-resume/ai-templates/[templateId]/page.tsx
"use client";
...
import TemplateRenderer from "../../../../app/components/TemplateRenderer";
import { generatePDF } from "../../../../../lib/pdfGenerator";
import { fontStyles, fontSizes, predefinedColors } from "../../../../../lib/resumeConstants";

export default function AITemplatePreview({ params }: { params: Promise<{ templateId: string }> }) {
  const { templateId, resumeData, setResumeData, setTemplateId } = useResumeStore();
  const router = useRouter();
  const [currentTemplateId, setCurrentTemplateId] = useState<string>("");
  const [isEditing, setIsEditing] = useState(false);
  const [editedData, setEditedData] = useState(resumeData);
  const [error, setError] = useState<string | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(resumeData.profileImage || null);

  // Debug and validate resumeData
  useEffect(() => {
    const resolveParams = async () => {
      const { templateId: paramTemplateId } = await params;
      setCurrentTemplateId(paramTemplateId);
    ...
    console.log("Resume Data in AITemplatePreview:", resumeData);
    console.log("localStorage resume-storage:", localStorage.getItem("resume-storage"));
    ...
    setEditedData(resumeData);
    setImagePreview(resumeData.profileImage || null);
  }, [templateId, params, router, resumeData, setTemplateId]);
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        setResumeData({ profileImage: result });
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleArrayChange = (field: keyof ResumeData, index: number, value: string) => {
    const updatedArray = [...((editedData[field] as string[]) || [])];
    updatedArray[index] = value;
    const filteredArray = updatedArray.filter((v) => v.trim() !== "");
    setEditedData((prev) => ({ ...prev, [field]: filteredArray.length > 0 ? filteredArray : [""] }));
    setResumeData({ [field]: filteredArray.length > 0 ? filteredArray : [""] });
  };
  ...
  const removeArrayField = (field: keyof ResumeData, index: number) => {
    const updatedArray = ((editedData[field] as string[]) || []).filter((_, i) => i !== index);
    setEditedData((prev) => ({ ...prev, [field]: updatedArray.length > 0 ? updatedArray : [""] }));
    setResumeData({ [field]: updatedArray.length > 0 ? updatedArray : [""] });
  };

  const renderField = (label: string, field: keyof ResumeData) => {
    if (isEditing) {
      if (Array.isArray(editedData[field])) {
        const arrayData = ((editedData[field] as string[]) || []).length > 0 ? (editedData[field] as string[]) : [""];
        return (
          <div className="space-y-3">
          ...
          {(field === "name" || field === "tag" || field === "summary") && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  value={editedData[`${field}FontStyle` as keyof ResumeData] || "regular"}
                  onChange={(e) => {
                    const newStyle = e.target.value as "regular" | "bold" | "italic" | "bold-italic";
                    setEditedData({ ...editedData, [`${field}FontStyle`]: newStyle });
                    setResumeData({ [`${field}FontStyle`]: newStyle });
                  }}
                  className="w-full px-2 py-1 border border-gray-300 rounded text-xs"
                >
                  {fontStyles.map((style) => (
              ...
              <div>
                ...
                <select
                  value={editedData[`${field}FontSize` as keyof ResumeData] || 12}
                  onChange={(e) => {
                    const newSize = parseInt(e.target.value);
                    setEditedData({ ...editedData, [`${field}FontSize`]: newSize });
                    setResumeData({ [`${field}FontSize`]: newSize });
                  }}
                  className="w-full px-2 py-1 border border-gray-300 rounded text-xs"
                >
                  {fontSizes.map((size) => (
    ...
    return (
      <div className="py-2 border-b border-gray-100">
        ...
        {(field === "name" || field === "tag" || field === "summary") && (
          <div className="mt-1 text-xs text-gray-500">
            Style: {resumeData[`${field}FontStyle` as keyof ResumeData] || "Regular"} | 
            Size: {resumeData[`${field}FontSize` as keyof ResumeData] || 12}px
          </div>
        )}
      </div>
    );
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                <div className="w-32 bg-gray-200 rounded-full h-2 mr-3">
                  <div
                    className="bg-gradient-to-r from-blue-500 to-purple-600 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${Math.min(100, Object.keys(editedData).filter(key => editedData[key as keyof ResumeData]).length * 5)}%` }}
                  ></div>
                </div>
                <span className="text-sm text-gray-600">
                  {Math.min(100, Object.keys(editedData).filter(key => editedData[key as keyof ResumeData]).length * 5)}% Complete
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex space-x-8">
            <button
              onClick={() => setIsEditing(true)}
              className={`flex items-center py-4 px-1 border-b-2 font-medium text-sm transition-colors ${
                isEditing ? "border-blue-500 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
              }`}
            >
              Edit Resume
            </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {error && (
          <motion.div
            ...
            role="alert"
          >
            {error} <Link href="/upload-resume" className="text-blue-600 underline">Upload a resume</Link> to continue.
          </motion.div>
        )}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <motion.div
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  {imagePreview && (
                    <div className="mt-4 flex items-center gap-4">
                      ...
                      <button
                        onClick={() => {
                          setEditedData((prev) => ({ ...prev, profileImage: null }));
                          setImagePreview(null);
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-8 gap-2 mb-4">
                    {predefinedColors.map((color) => (
                      <button
                        ...
                        onClick={() => handleColorSelect(color)}
                        className={`w-8 h-8 rounded-lg border-2 transition-all ${
                          editedData.headerColor === color ? "border-blue-500 scale-110" : "border-gray-300 hover:border-gray-400"
                        }`}
                        style={{ backgroundColor: color }}
                        aria-label={`Select header color ${color}`}
                      />
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div
                      className="w-8 h-8 rounded-lg border border-gray-300"
                      style={{
                        backgroundColor: /^#[0-9A-Fa-f]{6}$/.test(editedData.headerColor || "")
                          ? editedData.headerColor
                          : "#ffffff",
                      }}
                    />
                  </div>
                  {editedData.headerColor && !/^#[0-9A-Fa-f]{6}$/.test(editedData.headerColor) && (
                    <p className="text-red-500 text-xs mt-2">
                      Please enter a valid hex color (e.g., #FFFFFF).
                    </p>
                  )}
            ...
            <div className="p-6">
              ...
              <div className="flex justify-center">
                <button
                  onClick={async () => {
                    const filename = `${editedData.name || "resume"}-ai-template${currentTemplateId}.pdf`;
                    await generatePDF(editedData, filename, currentTemplateId, true);
                  }}
                  className="w-full flex items-center justify-center px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg hover:from-blue-600 hover:to-purple-700 transition-all duration-300"
                  disabled={!resumeData || Object.keys(resumeData).length === 0}
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
