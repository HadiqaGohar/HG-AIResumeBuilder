...
// //         toast.success("Resume uploaded with some missing information. You can edit it later.");
// //       } else {
// //         toast.dismiss("upload");
...
// //       // Navigate based on whether template was selected
// //       if (selectedTemplate) {
...
// //       setUploadError(error.message || "Failed to process your resume. Please try again.");
// //       toast.dismiss("upload");
// //       toast.error(error.message || "Failed to process your resume");
// //     } finally {
...
// //             Upload your existing resume and let our AI transform it into a professional masterpiece
// //           </p>
// //         </div>
...
// //               This may take a few moments while we extract and structure your information
// //             </p>
// //           </div>
// //         )}
...
export default function UploadResume() {
  const { setResumeData, setTemplateId, clearResumeData } = useResumeStore();
  const router = useRouter();
  const { status } = useSession();
  const [selectedTemplate, setSelectedTemplate] = useState<string | null>(null);
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  ...
  const handleFileUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // Clear old resume data before new upload
        clearResumeData();
        ...
        // Ensure store update completes
        await new Promise<void>((resolve) => {
          setResumeData(extractedData);
          console.log("Stored resume data:", extractedData);
          resolve();
        });
        ...
        const targetPath = selectedTemplate
          ? `/resume/customize/${selectedTemplate}`
          : "/upload-resume/ai-templates";
        router.push(targetPath);
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : "Failed to process your resume.";
        console.error("Upload error:", err);
        setUploadError(message);
        toast.dismiss("upload");
        toast.error(message);
    ...
    [setResumeData, clearResumeData, router, selectedTemplate]
  );
  ...
  const removeFile = () => {
    setUploadedFile(null);
    setUploadError(null);
    clearResumeData(); // Clear store when removing file
  };
            ...
            {!uploadedFile ? (
              ...
              <div className="flex items-center justify-between bg-white rounded-xl p-4 border border-green-200">
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    <p className="font-semibold text-gray-800">{uploadedFile.name}</p>
                    <p className="text-sm text-gray-500">{apiUtils.formatFileSize(uploadedFile.size)}</p>
                  </div>
                </div>
                {!isProcessing && (
                  <button
