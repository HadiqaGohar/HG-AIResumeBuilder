...
const JobOptimizer: React.FC<JobOptimizerProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto p-3 sm:p-6 bg-white rounded-xl shadow-lg">
      <div className="flex flex-col sm:flex-row items-center mb-3 sm:mb-6">
        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-green-500 to-blue-500 rounded-full flex items-center justify-center mr-0 sm:mr-4 mb-2 sm:mb-0">
          <FiTarget className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
        </div>
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Job-Specific Optimization</h2>
      ...
      {!showResults ? (
        <div className="space-y-4 sm:space-y-6">
          <div>
            ...
            <textarea
              ...
              placeholder="Paste the job description here. Include requirements, responsibilities, and preferred qualifications..."
              rows={5}
              className="w-full px-2 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-xs sm:text-sm"
            />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mt-4 sm:mt-8 pt-4 sm:pt-8 border-t border-gray-200">
            ...
            <div className="text-center">
              ...
              <h3 className="font-semibold text-gray-800 mb-1 sm:mb-2 text-xs sm:text-sm">Job Matching</h3>
              <p className="text-xs sm:text-sm text-gray-600">
                Align your resume content with job requirements
              </p>
            </div>
        ...
        <div className="space-y-4 sm:space-y-8">
          <div className="flex flex-col sm:flex-row items-center justify-between">
            <h3 className="text-lg sm:text-xl font-semibold text-gray-800">Optimization Results</h3>
            <div className="flex space-x-2 sm:space-x-3 mt-2 sm:mt-0">
              <button
                onClick={applyOptimization}
                className="flex items-center px-3 sm:px-6 py-1 sm:py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-xs sm:text-sm"
              >
                <FiCheck className="w-3 sm:w-4 h-3 sm:h-4 mr-1" />
                Apply Changes
              </button>
              <button
                onClick={clearOptimization}
                className="px-3 sm:px-6 py-1 sm:py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-xs sm:text-sm"
              >
                Start Over
              </button>
            </div>
          ...
          {optimization?.optimized_summary && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-6">
              <h4 className="font-semibold text-blue-800 mb-2 sm:mb-3 flex items-center text-xs sm:text-sm">
                <FiArrowRight className="w-3 sm:w-4 h-3 sm:h-4 mr-1" />
                Optimized Professional Summary
              </h4>
              <div className="space-y-2 sm:space-y-4">
                <div>
                  <p className="text-xs sm:text-sm text-gray-600 mb-1">Current Summary:</p>
                  <p className="text-gray-800 bg-white p-2 sm:p-3 rounded border text-xs sm:text-sm">
                    {resumeData.summary || "No summary provided"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-gray-800 bg-blue-100 p-2 sm:p-3 rounded border border-blue-300 text-xs sm:text-sm">
                    {optimization.optimized_summary}
                  </p>
                </div>
              </div>
          ...
          {optimization?.suggested_skills?.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-6">
              <h4 className="font-semibold text-green-800 mb-2 sm:mb-3 flex items-center text-xs sm:text-sm">
                <FiZap className="w-3 sm:w-4 h-3 sm:h-4 mr-1" />
                Suggested Skills to Add
              </h4>
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {optimization.suggested_skills.map(
                  (skill: string, index: number) => (
                    <span
                      key={index}
                      className="px-2 sm:px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs sm:text-sm border border-green-300"
                    >
                      {skill}
                    </span>
                  )
          ...
          {optimization?.keyword_matches?.length > 0 && (
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 sm:p-6">
              <h4 className="font-semibold text-purple-800 mb-2 sm:mb-3 flex items-center text-xs sm:text-sm">
                <FiTarget className="w-3 sm:w-4 h-3 sm:h-4 mr-1" />
                Matching Keywords Found
              </h4>
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {optimization.keyword_matches.map(
                  (keyword: string, index: number) => (
                    <span
                      key={index}
                      className="px-2 sm:px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-xs sm:text-sm border border-purple-300"
                    >
                      {keyword}
                    </span>
                  )
          ...
          {optimization?.improvement_suggestions?.length > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-6">
              <h4 className="font-semibold text-yellow-800 mb-2 sm:mb-3 flex items-center text-xs sm:text-sm">
                <FiTrendingUp className="w-3 sm:w-4 h-3 sm:h-4 mr-1" />
                Improvement Suggestions
              </h4>
              <ul className="space-y-1 sm:space-y-2">
                {optimization.improvement_suggestions.map(
                  (suggestion: string, index: number) => (
                    <li key={index} className="flex items-start">
                      <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-yellow-500 rounded-full mt-1 sm:mt-2 mr-2 sm:mr-3 flex-shrink-0"></span>
