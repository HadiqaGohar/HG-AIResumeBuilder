...
function Card({ icon, title, description, buttonText, href }: CardProps) {
  return (
    <div className="bg-transparent border-amber-50 border-2 rounded-2xl text-center flex flex-col">
      {/* Decorative lines */}
      <div className="flex justify-between px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8 py-1 sm:py-1 md:py-2">
        <div className="w-1/4 sm:w-1/4 md:w-1/3 lg:w-1/3 h-1 bg-amber-50"></div>
        <div className="w-1/4 sm:w-1/4 md:w-1/3 lg:w-1/3 h-1 bg-amber-50"></div>
      ...
      {/* Card Content */}
      <div className="p-2 sm:p-3 lg:p-6 xl:p-8 text-white flex flex-col flex-grow">
        {icon && <div className="text-2xl sm:text-2xl md:text-2xl lg:text-4xl xl:text-5xl mb-1 sm:mb-2 md:mb-2 lg:mb-2">{icon}</div>}
        <h2 className="text-2xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold mb-1 sm:mb-2 md:mb-2 lg:mb-2">{title}</h2>
        <p className="mb-2 sm:mb-3 md:mb-4 p-2 sm:p-3 md:p-4 text-sm sm:text-sm md:text-base lg:text-lg xl:text-xl flex-grow">{description}</p>
        
        {/* Link Button */}
        <div className="mt-auto">
          <Link href={href} passHref>
            <button className="bg-yellow-300 hover:bg-yellow-400 transition-colors rounded-full text-black px-4 sm:px-3 md:px-4 lg:px-8 xl:px-10 py-1 sm:py-2 md:py-2 lg:py-3 xl:py-3 border border-gray-800 text-sm sm:text-sm md:text-base lg:text-lg xl:text-xl w-full">
              {buttonText}
            </button>
          </Link>
        </div>
