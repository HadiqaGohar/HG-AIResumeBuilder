...
const SupportSection = () => {
  ...
  return (
    <section className="bg-white py-8 md:py-12 px-4 sm:px-6 md:px-8">
      ...
      <div className="max-w-6xl mx-auto text-center">
        {/* Header */}
        <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl uppercase mt-8 md:mt-16 lg:mt-20 font-light text-gray-900 mb-3 md:mb-4 animate-fadeInUp">
          Need Help? <span className="font-black">{"We're Here"} </span> for You
        </h2>
          ...
          {supportOptions.map((option, index) => {
            ...
            return (
              <div
                key={index}
                onClick={option.action}
                className="card bg-white rounded-lg md:rounded-xl shadow-sm md:shadow-md p-4 md:p-5 lg:p-6 border border-purple-100 md:border-2 md:border-purple-200 transition-all duration-300 hover:animate-liftHover cursor-pointer flex flex-col"
              >
                <IconComponent className="w-10 h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 mx-auto mb-3 md:mb-4 text-purple-600 transition-transform hover:rotate-12" aria-hidden="true" />
        ...
        <div className="mt-6 md:mt-8 lg:mt-10 animate-fadeInUp flex flex-col sm:flex-row justify-center gap-3 md:gap-4 lg:gap-6">
          <a
            href="mailto:support@resumecraft.com"
            className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 md:px-5 md:py-2.5 lg:px-6 lg:py-3 rounded-full transition-all duration-300 font-semibold text-sm md:text-base lg:text-lg hover:animate-liftHover inline-block text-center"
            aria-label="Email Support"
          >
            Email Support
          </a>
        ...
        <p className="text-gray-400 text-xs mt-4 md:mt-5 lg:mt-6 animate-fadeInUp">
          Support Available 24/7 - Last Updated: 04:24 PM PKT, Aug 03, 2025
        </p>
      </div>
